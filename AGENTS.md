...
- **Optional metadata cache**: Valkey/Redis via the async `redis` crate for exact-key and curated-index metadata acceleration
...
**Communication**: Services communicate via RabbitMQ for async operations; `attune-supervisor` is primarily DB-driven but uses RabbitMQ when configured to publish lifecycle wakeups after corrective remediation.
...
- **Policy**: First-class execution admission metadata stored in the `policy` table and managed via `GET/POST/PUT/DELETE /api/v1/policies`, web `/policies`, and `attune policy ...`. Policies may be global, pack-scoped, or action-scoped, with executor precedence of newest action policy → newest pack policy → newest global policy → no limit. The current policy fields model concurrency admission: `threshold` is the max concurrent executions, `method` is `enqueue` (FIFO wait) or `cancel` (reject when full), and `parameters` optionally defines parameter paths used to create per-group concurrency buckets. Policy API access uses the dedicated RBAC resource `policies` with actions `read`, `create`, `update`, and `delete`; list/detail operations post-filter by policy ref and pack constraints so unauthorized policy rows do not leak.
- **Rule**: Links triggers to actions with conditional logic. Carries an optional `owner_identity` column referencing `identity(id)` (`ON DELETE SET NULL`) — set to the authenticated user's id by `POST /api/v1/rules`, NULL for system-loaded rules from the init pack loader. Rules may define nullable `permission_set_refs` to override the target action's default execution API-token permission refs for rule-triggered executions; omitted/NULL inherits the action default and an empty array forces no execution token. The executor uses `rule.owner_identity` to attribute rule-triggered executions; legacy/system-loaded rules with NULL `owner_identity` fall back to the system identity (id 1) at execution-creation time.
- **Enforcement**: Represents a rule activation. Enforcement payload is a copy of the stored event payload, so event-secret fields remain redacted and are not restored by the enforcement decrypt path. Enforcement config redaction/reveal remains action-schema based; `GET /api/v1/enforcements/{id}?include_secret_values=true` requires `enforcements:decrypt` and restores only enforcement config secrets.
...
- **Key Settings**:
  ...
  - `metadata_cache` - Optional Valkey/Redis-backed cache for relatively static metadata rows (`action`, `rule`, `trigger`, `sensor`, `work_queue`, `workflow_definition`, `policy`, `permission_set`, `runtime`, `runtime_version`). When enabled with a URL, services use `attune_common::metadata_cache` for best-effort read-through/write-through caching, with `max_connections` controlling the number of Redis/Valkey connection managers each service creates. PostgreSQL remains authoritative: API and pack/workflow/runtime lifecycle writes update the cache after the DB commit, and API/executor/worker/sensor services run a `metadata_changed` LISTEN/NOTIFY repair listener for cross-process refresh/eviction. Curated indexes cover hot paths such as webhook-key trigger lookup, enabled work-queue polling, runtime name/alias/version lookup, and selected permission-set assignment reads; cache failures must never fail metadata writes. Process-local L1 cache exists even when Valkey is disabled; authenticated operators with `retention:read` can inspect bounded in-process cache counters at `GET /api/v1/diagnostics/metadata-cache`.
...
- **Modules**: `models`, `repositories`, `db`, `config`, `error`, `mq`, `crypto`, `utils`, `workflow` (includes `expression` sub-module), `pack_registry`, `template_resolver`, `version_matching`, `runtime_detection`, `scheduling`
...
attune policy list                 # List execution admission policies
attune policy create --ref core.echo_limit --name "Limit echo" --action core.echo --threshold 3 --method enqueue
attune policy update core.echo_limit --threshold 5
attune policy delete core.echo_limit --yes
...
- `scripts/run-metadata-cache-benchmark.sh` - Docker Compose benchmark runner for cache-on/cache-off comparisons
- `tests/benchmarks/metadata_cache_benchmark.py` - Blended metadata-cache benchmark driver and comparison report generator. Supports profiles (`smoke`, `metadata-heavy`, `automation-hotpath`, `execution-throughput`, `runtime-queue`, `stable`, `soak`) and reports process-local L1, Valkey L2, and Valkey server counters separately. The `soak` profile adds a multi-window `steady_state_hot_path` workload for longer-term warm-cache behavior and p95 drift analysis.
...
|docs/deployment:{ops-runbook-queues.md,production-deployment.md,supervisor.md}
...
|docs/performance:{QUICKREF-performance-optimization.md,log-size-limits.md,metadata-cache-benchmark.md,metadata-cache-results-template.md,performance-analysis-workflow-lists.md,performance-before-after-results.md,performance-context-cloning-diagram.md}
