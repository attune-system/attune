...
owners will require their matching reference; identity ownership will resolve to
This matches the existing CLI and cache API ownership model.
...
## Intentionally Excluded Bulk Operations
...
Those CLI operations are designed for streaming arbitrary datasets through
...
The bounded scan and explicit refresh-lifecycle tools provide the same
capability safely over multiple calls.
...
authorization. The API remains responsible for cache RBAC, signed sensor-cache
must state that cache values can contain sensitive business data and that
refresh writes require explicit cache-write grants.
